...
export class SalesServiceProxy {
     ...
     * @return Success
     */
    getAllTempSales(keyword: string | null | undefined, isActive: boolean | null | undefined, skipCount: number | undefined, maxResultCount: number | undefined): Observable<TempSaleDtoPagedResultDto> {
        let url_ = this.baseUrl + "/api/services/app/Sales/GetAllTempSales?";
        if (keyword !== undefined && keyword !== null)
            url_ += "Keyword=" + encodeURIComponent("" + keyword) + "&";
        if (isActive !== undefined && isActive !== null)
            url_ += "IsActive=" + encodeURIComponent("" + isActive) + "&";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetAllTempSales(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAllTempSales(<any>response_);
                } catch (e) {
                    return <Observable<TempSaleDtoPagedResultDto>><any>_observableThrow(e);
                }
            } else
                return <Observable<TempSaleDtoPagedResultDto>><any>_observableThrow(response_);
        }));
    }

    protected processGetAllTempSales(response: HttpResponseBase): Observable<TempSaleDtoPagedResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = TempSaleDtoPagedResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<TempSaleDtoPagedResultDto>(<any>null);
    }
...
export class TempSaleDtoPagedResultDto implements ITempSaleDtoPagedResultDto {
    totalCount: number;
    items: TempSaleDto[] | undefined;

    constructor(data?: ITempSaleDtoPagedResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["items"])) {
                this.items = [] as any;
                for (let item of _data["items"])
                    this.items.push(TempSaleDto.fromJS(item));
            }
        }
    }

    static fromJS(data: any): TempSaleDtoPagedResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new TempSaleDtoPagedResultDto();
        result.init(data);
        return result;
    }
    ...
    clone(): TempSaleDtoPagedResultDto {
        const json = this.toJSON();
        let result = new TempSaleDtoPagedResultDto();
        result.init(json);
        return result;
...
export interface ITempSaleDtoPagedResultDto {
    totalCount: number;
    items: TempSaleDto[] | undefined;
}
